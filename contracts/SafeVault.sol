...
contract SafeVault is ISafeVault, Proxied, ReentrancyGuard {
    ...
    function setSafeToken(address _safeToken) external onlyProxyAdmin {
    ...
    function remove(address _receiver, uint256 _amount) external nonReentrant {
        ...
        /// @dev tx.origin is probably not the best way to check, although it's the only way to check the original proxy here since msg.sender is the implementation contract
    ...
    function recoverERC20(address tokenAddress, uint256 tokenAmount) public onlyProxyAdmin {
        IERC20(tokenAddress).transfer(_proxyAdmin(), tokenAmount);
    }

    function recoverETH() external onlyProxyAdmin {
