...
export default task('airdrop', 'distribute Safe NFTs according to the list').setAction(async (_, hre) => {
  ...
  for (const data of airdropData) {
    ...
    for (let i = 0; i < tier.length; i++) {
      ...
      info(`Minting ${tier[i]} tier ${i + 1} NFTs to ${to} ...`);
      const tx = await nftContract.mint(to as string, i, tier[i], []);
      await tx.wait();
    }
  }
  success('Done.');
