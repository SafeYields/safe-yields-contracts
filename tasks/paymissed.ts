import { SafeNFT, SafeRouter } from '@contractTypes/contracts';
import { formattedFromBigNumber, info, networkInfo, success } from '@utils/output.helper';
...
export default task('paymissed', 'add wallets back to the rewards distribution list').setAction(async (_, hre) => {
  await networkInfo(hre, info);
  const nftContract = await hre.ethers.getContract<SafeNFT>('SafeNFT');
  const total = (await nftContract.getTotalSupplyAllTiers())
    .map((t, i) => t.toNumber() * (i + 1))
    .reduce((a, b) => a + b, 0);
  const addresses = [...new Set(airdropData.map(data => data[0].toString().toLowerCase()))];
  const addressesWithValue: { address: string; share: number }[] = await Promise.all(
    addresses.map(async data => {
      return {
        address: data,
        share: Number(await nftContract.votingPower(data.toString().toLowerCase())),
      };
    }),
  );
  const totalShare = addressesWithValue.reduce((a, b) => a + b.share, 0);
  const amount = 584460000;
  info(
    `Total amount ${amount} (${formattedFromBigNumber(
      amount,
    )}), total share ${totalShare}, among total supply: ${total}...`,
  );
  const amountToDistribute = Math.floor((amount * totalShare) / total);
  info(`Amount to distribute ${amountToDistribute} (${formattedFromBigNumber(amountToDistribute)})`);
  const addressesWithAmount = addressesWithValue.map(({ address, share }) => [
    address,
    Math.floor((amount * share) / total),
  ]);
  addressesWithAmount.forEach(([address, amount]) => info(`${address} ${formattedFromBigNumber(amount)}`));

  const receivers = addressesWithAmount.map(i => i[0] as string);
  const amounts = addressesWithAmount.map(i => i[1]);
