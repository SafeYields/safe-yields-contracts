...
export default task('blacklist', 'exclude wallets from rewards distribution list').setAction(async (_, hre) => {
  ...
  info(`Excluding ${addresses.length} addresses from rewards distribution ...`);
  const tx = await nftContract.blacklistForRewardsDistribution(addresses);
  await tx.wait();
  success('Done.');
});
